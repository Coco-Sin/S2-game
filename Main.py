...
### récupérer le temps présent du minuteur 
def getTime(minuteur) :
	return minuteur["actualTime"] 
	pass

### definition de setTime (réglage)
def setTime(minuteur, nb_sec) :
...
def move():
    ...
    if 1<int(MyPlayer['y'])<26:
      ...
      ### collision avec les X ~ on renvoie la valeur renvoyée par chaque énigme dans le moudule interactEnigmes, selon la position de la croix associée
...
### intéractions possibles pour le joueur pour accéder à la carte, les aides ou pour se déplacer
def interact():

    timerEG.minuteur(monMinuteur)

    global Player, Background, Help, Affichage, grille, Information, timerEG, lagrossedefaite
    if isDataReady():
	c = sys.stdin.read(1)
        sys.stderr.write(str(monMinuteur))
        if c == '\x1b':
            QuitGame()
        elif c == 'm':
	    Affichage = "back"
	    ...
	    Player.MoveLeft(MyPlayer)
            ...
            Affichage = "info"
    if monMinuteur["actualTime"] < 0:
        Affichage = "lose"
        sys.stderr.write("patate %d" %(monMinuteur["actualTime"],))
            
	
    return Affichage

### affichage du backgroud, de l'animat, du menu help et du minuteur
def show():
